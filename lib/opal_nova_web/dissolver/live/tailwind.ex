defmodule OpalNovaWeb.Dissolver.Live.Tailwind do
  use Phoenix.Component

  def paginate(assigns) do
    assigns =
      assigns
      |> assign_new(:page_list, fn %{route_helper: route_helper, dissolver: dissolver} ->
        opts = Dissolver.Paginator.build_options([])
        Dissolver.Paginator.paginate(dissolver, route_helper, opts)
      end)

    ~H"""
      <div class="text-center pagination mt-8" role="pagination">
      <%= for {label, _page, url, is_current} <- @page_list do %>
        <%= live_patch label, to: url, class:
...
# defmodule OpalNovaWeb.Dissolver.HTML.Tailwind do
